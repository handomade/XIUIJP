...
local function DrawProfilePopups()
    ...
    if (imgui.BeginPopupModal("Rename Profile", true, ImGuiWindowFlags_AlwaysAutoResize)) then
        isModalOpen = true;
        imgui.Text(string.format(i18n.T("Rename '%s' to:"), GetCurrentProfileName()));
...
config.DrawWindow = function(us)
    ...
    local configVisible = imgui.Begin("XIUI Config##xiuiConfig v" .. addon.version, showConfig, bit.bor(ImGuiWindowFlags_NoSavedSettings, ImGuiWindowFlags_NoDocking));
    ...
    if configVisible then
        ...
        if creditsWindowOpen[1] then
            ...
            if imgui.Begin("Attributions###CreditsWindow", creditsWindowOpen, creditsFlags) then
                ...
                imgui.BulletText("Hando - Japanese overlay (XIUI JP)");
            end
            imgui.End();
