...
local function index_name(map, raw, id)
    ...
    if not map[lowered] then
        map[lowered] = id;
    end
    local utf = resnames.ToUtf8(raw);
    if utf ~= raw then
        if not map[utf] then
            map[utf] = id;
        end
        local utfLower = utf:lower();
        if not map[utfLower] then
            map[utfLower] = id;
        end
    end
end

local function index_spell(map, spell, id)
    if spell and spell.Name then
        for i = 1, 3 do
            index_name(map, spell.Name[i], id);
        end
    end
    local rm = AshitaCore:GetResourceManager();
    if rm then
        pcall(function()
            index_name(map, rm:GetString('spells.names', id, 1), id);
        end);
    end
end
...
local function BuildSpellLookup()
    ...
    for id = 0, 1024 do
        ...
        if spell and spell.Name and spell.Name[1] then
            index_spell(M.spellNameToId, spell, id);
        end
    end
end
...
local function BuildAbilityLookup()
    ...
    for id = 0, 2048 do
        ...
        if ability and ability.Name and ability.Name[1] then
            ...
            for i = 1, 3 do
                index_name(M.abilityNameToId, ability.Name[i], M.abilityNameToId[name] or id);
            end
            pcall(function()
                index_name(M.abilityNameToId, AshitaCore:GetResourceManager():GetString('abilities.names', id, 1), M.abilityNameToId[name] or id);
            end);
            ...
            if score > (petBestScore[name] or -1) then
                ...
                for i = 1, 3 do
                    index_name(M.abilityNameToPetId, ability.Name[i], id);
                end
            end
        end
    end
...
local function lookup(map, name)
    if not name then return nil; end
    return map[name] or map[name:lower()] or map[resnames.ToUtf8(name)] or map[resnames.ToUtf8(name):lower()];
end

-- Get spell ID by name
function M.GetSpellId(spellName)
    BuildSpellLookup();
    return lookup(M.spellNameToId, spellName);
end

-- Get ability ID by name (job abilities / weaponskills)
function M.GetAbilityId(abilityName)
    BuildAbilityLookup();
    return lookup(M.abilityNameToId, abilityName);
end
...
function M.GetPetAbilityId(abilityName)
    ...
    BuildAbilityLookup();
    return lookup(M.abilityNameToPetId, abilityName) or lookup(M.abilityNameToId, abilityName);
end
