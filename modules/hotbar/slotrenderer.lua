...
function M.DrawSlot(params)
    ...
    if params.showLabel and params.labelText and params.labelText ~= '' and animOpacity > 0.5 and drawList then
        ...
        local lines = GetLabelLayout(labelSrc, lblFontSize, maxW, params.labelWordWrap);
        for i = 1, #lines do
            local line = lines[i];
...
function M.FlushLabels()
    ...
    for i = 1, pendingLabelCount do
        ...
        imtext.Draw(entry.drawList, resnames.ToUtf8(entry.text), entry.x, entry.y, entry.color, entry.fontSize);
        entry.drawList = nil;
        entry.text = nil;
    end
    pendingLabelCount = 0;
