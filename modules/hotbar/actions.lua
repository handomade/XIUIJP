...
function M.BuildCommand(bind)
    ...
    local quoted = function(name)
        local jp = resnames.PreferJapanese(bind.actionType, name or '');
        return '"' .. resnames.ForCommand(jp) .. '"';
    end;

    -- Build command based on action type
    if bind.actionType == 'ma' then
        command = '/ma ' .. quoted(bind.action) .. ' ' .. target;
    elseif bind.actionType == 'ja' then
        command = '/ja ' .. quoted(bind.action) .. ' ' .. target;
    elseif bind.actionType == 'ws' then
        command = '/ws ' .. quoted(bind.action) .. ' ' .. target;
    elseif bind.actionType == 'item' then
        command = '/item ' .. quoted(bind.action) .. ' ' .. target;
    elseif bind.actionType == 'equip' then
        local slot = bind.equipSlot or 'main';
        command = '/equip ' .. slot .. ' ' .. quoted(bind.action);
    elseif bind.actionType == 'pet' then
        command = '/pet ' .. quoted(bind.action) .. ' ' .. target;
            ...
            if chatManager then
                for i = #lines, 1, -1 do
                    ...
                    if trimmed and trimmed ~= '' then
                        chatManager:QueueCommand(2, resnames.ForCommand(trimmed));
                    end
                end
            end
        end);
        ...
        if waitMatch then
            ...
            local function queueActionLine()
                ...
                local ok, err = pcall(function()
                    local chatManager = AshitaCore:GetChatManager();
                    if chatManager then
                        chatManager:QueueCommand(cmdMode, resnames.ForCommand(commandToExecute));
                    end
                end);
